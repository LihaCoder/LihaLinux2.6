...
static struct task_struct *dup_task_struct(struct task_struct *orig)
{
	...
	// orig是当前运行的进程
	// 目前的理解这里是停止内核抢占，并且改变cr0中某些状态位。（好难顶啊）
	...
	// 得到新的页表的地址 
	// 将当前进程的thread_info赋值到得到的内存地址中。
	*ti = *orig->thread_info;

	// 得到新的页表的地址 
	// 将当前进程的task_struct赋值到得到的内存地址中。
	*tsk = *orig;
				 ...
				 int __user *child_tidptr)
{
	...
	// CLONE_NEWNS|CLONE_FS 二进制组合以后 
	// 与clone_flags二进制截断，如果等于CLONE_NEWNS|CLONE_FS
	// 那么真像只有一个，clone_flags等于CLONE_NEWNS|CLONE_FS
	 ...
	 */
	// 如果clone_flags和CLONE_THREAD相等
	// 并且 clone_flags与CLONE_SIGHAND完全相反。
	 ...
	 */
	// 只要clone_flags 与 CLONE_SIGHAND不是完全相反
	// clone_flags与CLONE_VM完全相反
	...
	// dup_task_struct找到两个页，一个存放thread_info结构体，一个存放task_struct结构体
	// 并且把父进程的thread_info和task_struct结构体 找到新开的页，然后进行赋值。
	// 最后返回的子进程的task_struct的那个页表的地址返回。
