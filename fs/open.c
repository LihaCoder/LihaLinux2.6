 ...
 * used by symlinks.
 */
struct file *filp_open(const ch ar * filename, int flags, int mode)
{
	...
	// 看到&nd，就能明白内部肯定是对nameidata结构体进行操作。
	// 而nameidata结构体中有dentry，vfsmount
...
struct file *dentry_open(struct dentry *dentry, struct vfsmount *mnt, int flags)
{
	...
	// 从inode里面拿到的f_op，所以要找到inode的初始化。
